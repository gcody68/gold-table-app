...
export default function OrderCustomizationModal({ item, onClose }: Props) {
  ...
  const handleAddToOrder = () => {
    ...
    addItem(item, specialInstructions.trim() || undefined);
    onClose();
  };
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="bg-card border-border sm:max-w-md">
        ...
        <div className="space-y-1 border border-border rounded-lg p-3 bg-secondary/40">
          ...
          <p className="text-gold text-sm font-medium">${Number(item.price).toFixed(2)}</p>
          {item.description && (
