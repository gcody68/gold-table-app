...
export default function MenuGrid() {
  ...
  // Filter: admin sees all, customers see only non-placeholders
  const visibleItems = isAdmin ? items : items?.filter((i) => !i.is_placeholder);
        ...
        {visibleItems?.map((item, i) => {
          if (item.is_placeholder) {
            ...
            return (
              <div
                key={item.id}
                className="rounded-lg overflow-hidden bg-card border border-dashed border-border hover:border-gold/30 transition-all duration-300 animate-fade-in"
